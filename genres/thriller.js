...
async function fetchThriller() {
    const url = "https://openlibrary.org/subjects/thrillers.json?limit=50"
    const response = await fetch(url)
    const data = await response.json()
    
    let thrillerBooks = data.works.map(book => {
        console.log(book)
...
function displayThriller(thrillerBooks) {
    ...
    for (let book of thrillerBooks) {
        ...
        card.innerHTML = `
            ...
            `
        thrillerBooksContainer.appendChild(card)
